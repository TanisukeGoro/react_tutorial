...
const ItemList = (props) => {
  ...
  const getTodosFromFirestore = async () => {
    const itemListArray = await firebase.firestore().collection('todos').orderBy('isDone').orderBy('limit').get()
    ...
    <div>
      ...
      <ul>
        {todoList?.map((x, index) => (
          <Item key={index} todo={x} index={index} getTodosFromFirestore={getTodosFromFirestore} />
        ))}
      </ul>
    </div>
  )
