...
const Item = ({ index, todo, getTodosFromFirestore }) => {
  ...
  const deleteDataOnFirestore = async (collectionName, documentId) => {
    const removedData = await firebase.firestore().collection(collectionName).doc(documentId).delete()
    getTodosFromFirestore()
    return
  }
  ...
  return (
    <li key={index} id={todo.id}>
      ...
      />
      <button value={todo.id} onClick={(e) => deleteDataOnFirestore('todos', todo.id)}>
        delete
      </button>
      {!todo.data.isDone ? (
        <div>
        ...
        <div>
          <p>
            <del>締め切り：{convertFromTimestampToDatetime(todo.data.limit.seconds)}</del>
          </p>
          <p>
